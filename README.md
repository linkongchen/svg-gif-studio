...
2. 上传 SVG 文件，或粘贴 SVG、HTML + CSS 动画代码，预览并导出。

输出格式：有损 GIF（64/128/256 色）、无损 APNG（完整 RGBA 色彩）、动画 WebP（高画质有损编码），也可同时生成。可调整预览颜色、背景（包括透明）、大小、帧率和录制时长；自定义录制时长支持 100～20000 毫秒的整数，最多录制 240 帧。APNG 和 WebP 的帧时长以毫秒保存；GIF 格式只能精确到 10 毫秒。文件在浏览器内编码，导入的代码不会上传。
...
扫描的是网页返回的 HTML 和引用 SVG；其他站点由 JavaScript 运行后动态生成的 SVG 不会自动执行或扫描。LDRS 是特例。外部图片、脚本和远程资源会从导入的动画代码中移除。对自包含的内联 CSS / SMIL 动画可录制；浏览器不支持的效果可能与原图不同。

GIF 格式最多 256 色，透明度只有透明或不透明两档，无法保证原始 SVG 的色彩无损。无损 APNG 指录制得到的每帧像素以无损 PNG 保存，不代表与矢量 SVG 的分辨率无关。
